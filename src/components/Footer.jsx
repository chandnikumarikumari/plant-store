const Footer = () => (
  <footer className="bg-flora-bg2 border-t border-flora-border" style={{ padding: "60px 60px 30px" }}>
    ...
    >
      <div>
        <span className="text-flora-green text-xs font-semibold uppercase tracking-widest mb-2 block">
          ✦ For Every Update
        </span>
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
      <div>
        ...
        <p className="text-flora-muted text-xs leading-relaxed mb-6">
          From lush indoor greens to vibrant flowering plants — FloraVision brings
          nature's finest to your doorstep, with love and care in every leaf.
        </p>
        <div className="flex gap-2.5">
          {[
            ...
            <button
              ...
              type="button"
              aria-label={s.label}
              className="bg-flora-card border border-flora-border rounded-lg w-9 h-9 flex items-center justify-center text-sm hover:bg-flora-green hover:border-flora-green transition-all duration-200 cursor-pointer"
            >
              {s.icon}
            </button>
          ))}
          ...
          <ul className="list-none flex flex-col gap-2.5">
            {col.links.map((l) => (
              <li key={l}>
                <button
                  type="button"
                  className="text-flora-muted text-xs hover:text-flora-green transition-colors bg-transparent border-none p-0 cursor-pointer text-left"
                >
                  {l}
                </button>
              </li>
      ...
      <div className="flex gap-6">
        {["Privacy", "Terms", "Cookies"].map((l) => (
          <button
            key={l}
            type="button"
            className="text-flora-muted text-xs hover:text-flora-green transition-colors bg-transparent border-none p-0 cursor-pointer"
          >
            {l}
          </button>
        ))}
